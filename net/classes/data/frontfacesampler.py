...
class FrontFaceSampler(CDataset):
    """ Same as cloud sampler but only sample z > 0 """
    ...
    def _init(self):
        ...
        self._on_front_mask = self._coords[:,2] > 0
        self._front_count = self._on_front_mask.sum()
        self._has_init = True
        ...
        if self.sample_sdf:
            ...
            if os.path.isfile(self._cache_file):
                ...
                self._off_samples = self._off_samples[self._off_samples[:, 2] > 0]
                self._off_sdf = self._off_sdf[self._off_samples[:, 2] > 0]
                self.runner.py_logger.info(f'Filtered z < 0 points resulting in {self._off_samples.shape[0]} SDF samples.')
            else:
                #
                ...
                self._off_front_mask = self._off_samples[:, 2] > 0
                self._off_front_count = self._off_front_mask.sum()
    ...
    def epoch_hook(self, progress):
        tmp = progress >= self.base_part
        if self._only_front != tmp:
            self.runner.py_logger.info(f"{self.__class__.__name__} sample front: {tmp}")
            self.runner.py_logger.info(f"{self.__class__.__name__} sample close to surface: {tmp}")
        self._only_front = tmp
        # self.sample_close_to_surface = tmp
    ...
    def __getitem__(self, idx):
        ...
        if self._only_front:
            rand_idcs = np.random.choice(self._front_count,
                                                size=on_surface_samples)
            on_surface_coords = self._coords[self._on_front_mask,:][rand_idcs, :]
            on_surface_normals = self._normals[self._on_front_mask,:][rand_idcs, :]
        else:
        ...
        if off_surface_samples > 0:
            if self.sample_sdf:
                # if self._only_front:
                #     rnd_idx = np.random.choice(self._off_front_count, off_surface_samples)
                #     off_surface_coords = self._off_samples[self._off_front_mask][rnd_idx, :]
                #     sdf[on_surface_samples:, :] = self._off_sdf[self._off_front_mask][rnd_idx, :]
                # else:
                ...
                if self.sample_close_to_surface:
                    # if self._only_front:
                    #     rnd_idx = np.random.choice(self._front_count, off_surface_samples)
                    #     off_surface_coords = self._coords[self._on_front_mask, :][rnd_idx,:]
                    # else:
                    rnd_idx = np.random.choice(point_cloud_size, off_surface_samples)
                    off_surface_coords = self._coords[rnd_idx, :]
                    off_surface_coords += torch.randn_like(off_surface_coords) * 0.05
                else:
                    off_surface_coords = np.random.uniform(-0.5, 0.5, size=(off_surface_samples, 3)) * self.bbox_size

                    # if self._only_front:
                    #     mask = off_surface_coords[:, 2] <= 0
                    #     tmp = off_surface_coords[mask]
                    #     tmp[:, 2] *= -1
                    #     off_surface_coords[mask] = tmp
